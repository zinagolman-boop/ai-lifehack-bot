...
SEND_HOUR = int(os.environ.get("SEND_HOUR", "9"))  # час отправки (по UTC)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user.get("level"):
        await update.message.reply_text(
            ...
            f"Лайфхаки приходят каждое утро.\n"
            f"Используй /lifehack — получить лайфхак прямо сейчас\n"
        ...
        await update.message.reply_text(
            ...
            "Сначала давай я оценю твои знания. Пройди короткий тест — 3 вопроса.\n\n"
            "Готов(а)? Нажми кнопку 👇",
...
async def send_assessment_question(query_or_message, user_id: str, step: int):
    questions = [
        {
            "q": "Как часто ты пользуешься ChatGPT, Claude или другими ИИ?",
            "options": ["Никогда не пробовал(а)", "Иногда, но поверхностно", "Регулярно использую", "Использую каждый день для работы"]
        },
        {
            "q": "Что такое промпт?",
            "options": ["Не знаю", "Слышал(а), но не уверен(а)", "Знаю — это запрос к ИИ", "Знаю и умею писать эффективные промпты"]
        },
        {
            "q": "Пробовал(а) ли ты использовать ИИ для работы или учёбы?",
            "options": ["Нет", "Один-два раза", "Да, периодически помогает", "Да, это часть моего рабочего процесса"]
        }
    ]
...
async def finalize_assessment(query, user_id: str):
    user = get_user(user_id) or {}
    answers = user.get("assessment_answers", [0, 0, 0])
    total = sum(answers)
    
    if total <= 2:
        level = "новичок"
        desc = "Ты только начинаешь знакомство с ИИ — это отлично! Начнём с самых основ."
    elif total <= 5:
        level = "любопытный"
        desc = "Ты знаком(а) с ИИ, но есть много интересного что ещё предстоит открыть."
    elif total <= 7:
        level = "практик"
        desc = "Ты уже используешь ИИ на практике. Будем углублять и оттачивать навыки."
    else:
        level = "продвинутый"
        desc = "Ты хорошо разбираешься в ИИ. Сосредоточимся на продвинутых техниках."
    ...
    await query.edit_message_text(
        ...
        f"{desc}\n\n"
        f"Каждое утро в 9:00 буду присылать тебе новый лайфхак.\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(get_lifehack_now_callback, pattern="^get_lifehack_now$"))
